...
compatibility_threshold_adjustment = config.get(
    'compatibility_threshold_adjustment')  # 類似度の閾値の調整
dynamic_rate = float(config.get('dynamic_rate'))  # 類似度の閾値の調整率
ideal_species_num = int(config.get('ideal_species_num'))  # 理想的な種数
c1 = c2 = float(config.get('c1'))  # 類似度の計算係数
c3 = float(config.get('c3'))  # 類似度の計算係数
kp = float(config.get('kp'))  # PI制御のPゲイン
ki = float(config.get('ki'))  # PI制御のIゲイン
...
class DefaultSpeciesSet:
    ...
    def __init__(self, compatibility_threshold=1.0):
        """
        コンストラクタ
        """
        ...
        self.compatibility_threshold = compatibility_threshold  # 類似度の閾値
        self.accumulated_error = 0  # 累積誤差
    ...
    def speciate(self, population, generation):
        ...
        for genome_id in population.keys():
            ...
            for species_id, species in self.species.items():
                if self.distance(population[genome_id], species.representative) < self.compatibility_threshold:
        ...
        if compatibility_threshold_adjustment == 'fixed':
            pass
        elif compatibility_threshold_adjustment == 'dynamic':
            if len(self.species) < ideal_species_num:
                self.compatibility_threshold = max(
                    0, self.compatibility_threshold - dynamic_rate)
            elif len(self.species) > ideal_species_num:
                self.compatibility_threshold += dynamic_rate
        elif compatibility_threshold_adjustment == 'pi':
            error = ideal_species_num - len(self.species)
            self.accumulated_error += error
            self.compatibility_threshold += kp * error + ki * self.accumulated_error
            self.compatibility_threshold = max(0, self.compatibility_threshold)
